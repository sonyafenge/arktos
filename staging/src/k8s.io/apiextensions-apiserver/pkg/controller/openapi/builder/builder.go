...
func BuildSwagger(crd *apiextensions.CustomResourceDefinition, version string) (*spec.Swagger, error) {
	...
	if b.namespaced && b.tenanted {
		routes = append(routes, b.buildRoute(root, "", "GET", "list", "list", sampleList).Operation("list"+b.kind+"ForAllNamespacesAllTenants"))

		root = fmt.Sprintf("/apis/%s/%s/tenants/{tenant}/%s", b.group, b.version, b.plural)
		routes = append(routes, b.buildRoute(root, "", "GET", "list", "list", sampleList).Operation("list"+b.kind+"ForAllNamespacesUnderOneTenatn"))

		root = fmt.Sprintf("/apis/%s/%s/tenants/{tenant}/namespaces/{namespace}/%s", b.group, b.version, b.plural)
	}

	if !b.namespaced && b.tenanted {
		routes = append(routes, b.buildRoute(root, "", "GET", "list", "list", sampleList).Operation("list"+b.kind+"ForAllTenants"))
		root = fmt.Sprintf("/apis/%s/%s/tenants/{tenant}/%s", b.group, b.version, b.plural)
	}
