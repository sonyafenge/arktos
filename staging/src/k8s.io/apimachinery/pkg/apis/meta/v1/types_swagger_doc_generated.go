...
var map_ListOptions = map[string]string{
	...
	"allowPartialWatch":   "Whether needs to watch all api server data partition. Used to for some data that is only available for one partition. Hence could fail on watching other partition Generally used in client only to indicate partial failure is allowed",
...
var map_ObjectMeta = map[string]string{
	...
	"hashKey":                    "HashKey is used to distribute workload. It is typically generated automatically by the server on successful creation of a resource and is not allowed to change on PUT operations.\n\nPopulated by the system. Read-only.",
...
var map_OwnerReference = map[string]string{
	"":                   "OwnerReference contains enough information to let you identify an owning object. An owning object must be in the same tenant/namespace as the dependent, or be cluster-scoped, so there is no tenant or namespace field.",
	...
	"hashKey":            "Hashkey of the referent.",
