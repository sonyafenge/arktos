...
func newCustomResourceDefinitionsWithMultiTenancy(c *ApiextensionsV1Client, tenant string) *customResourceDefinitions {
...
func (c *customResourceDefinitions) List(opts metav1.ListOptions) (result *v1.CustomResourceDefinitionList, err error) {
	...
	if wgLen > 1 {
		...
		wg.Add(wgLen)
		results := make(map[int]*v1.CustomResourceDefinitionList)
		errs := make(map[int]error)
		for i, client := range c.clients {
			go func(c *customResourceDefinitions, ci rest.Interface, opts metav1.ListOptions, lock *sync.Mutex, pos int, resultMap map[int]*v1.CustomResourceDefinitionList, errMap map[int]error) {
				r := &v1.CustomResourceDefinitionList{}
				err := ci.Get().
					Tenant(c.te).
					Resource("customresourcedefinitions").
		...
		itemsMap := make(map[string]v1.CustomResourceDefinition)
		for j := 0; j < wgLen; j++ {
